...
@section('content')
  ...
  <form action="{{ url('struktur/'.$struktur->id) }}/update" method="post" enctype="multipart/form-data" id="upload-file">
    @method("PUT")
  @csrf
...
<div class="card card-primary">
  <div class="card-header">
    <h3 class="card-title">Upload struktur Infoermasi</h3>
  </div>
  <div class="card-body">
    <div class="form-row">
      <div class="col-md-12 mb-12">
        <label for="exampleInputEmail1">Tulisan</label>
        <input type="hidden" name="id" value="{{ old('id') ?? $struktur->id }}" class="form-control" id="exampleInputEmail1" placeholder="Profil Desa">
          <textarea id="textarea" class="form-control" name="tulisan" value="" rows="10" cols="100">{{ old('ket') ?? $struktur->tulisan }}</textarea>
      </div>
      ...
      <div class="col-md-3 mb-3">
        ...
        <select name="tampil" class="form-control" id="exampleInputPassword1" placeholder="">
                <option>{{ old('tampil') ?? $struktur->tampil }}</option>
    ...
    <a href="{{ url('/struktur') }}" class="btn btn-default"> Batal</a>
